...
V1.4.4 延伸同題評分狀態到整組練習的最後一題。最後一題評分後不再呼叫獨立結果頁，而是在既有筆跡與該題成績下方加入整組平均分數及達標題數。完成資訊顯示後，底部按鈕只負責清理 Canvas 並回到練習設定。

V1.4.3 的「筆跡在上、分數在下」配置仍保留：評分狀態沿用同一個 HandwritingEngine 與 strokes，只依新的尺寸重繪。最後一題亦使用同一個 Canvas，不建立第二張預覽或重新計分。Google Drive 同步先排程，待畫布離開後執行，避免完成畫面造成書寫資料遺失或主執行緒卡頓。
...
GitHub Pages 提供 HTTPS 靜態前端；Google Identity 提供授權，Google Drive 儲存備份與裝置學習狀態；Cloudflare Worker、D1、Cron 負責每日推播與完成狀態核對。V1.4.4 未變更 Worker 或 D1，因此更新本版不需執行 `db:init` 或重新部署 Worker。
