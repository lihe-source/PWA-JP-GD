...
V1.4.2 將五十音手寫拆成「書寫」與「評分」兩種視覺狀態。書寫狀態保留大畫布；評分狀態在 iPhone 將成績卡移到捲動區最前方，原畫布預設收合，下一題操作列保留在底部導覽上方的獨立空間。切換只發生在完成評分後，不在落筆期間重排或縮放 Canvas。

評分狀態保留同一個 HandwritingEngine 與筆跡資料。展開筆跡時才要求引擎依目前可用尺寸重繪；收合期間不清除 strokes。換題時移除評分狀態、回到捲動起點，再由既有 `_resetWriterQuestion()` 清除並載入下一個假名。iPad 寬版仍採左右分欄，不套用手機收合規則。
...
GitHub Pages 提供 HTTPS 靜態前端；Google Identity 提供授權，Google Drive 儲存備份與裝置學習狀態；Cloudflare Worker、D1、Cron 負責每日推播與完成狀態核對。V1.4.2 未變更 Worker 或 D1，因此更新本版不需執行 `db:init` 或重新部署 Worker。
