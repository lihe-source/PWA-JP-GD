...
- App：`V1_2_10`
- 顯示：`V1.2.10`
- PWA Cache：`Japanese-PWA-V1_2_10`
- Cloudflare Worker：`V1.2.10`（通知與資料表邏輯未變更）
...
7. 開始練習時由使用者點擊事件同步呼叫 `Sound.unlock()` 與輸入框 `focus()`，保留 iOS／iPadOS 的音效及軟體鍵盤授權。
8. 每題答對／答錯分別呼叫單字拼寫既有的 `Sound.playCorrect()`／`Sound.playWrong()`；結果頁依正確率呼叫 `Sound.playResult()`。
9. 輸入框採 `inputmode="text"`、`lang="en"`、停用自動修正與自動大寫，適合輸入羅馬拼音。
10. 練習期間只建立一次輸入框；換題僅更新文字、進度、ARIA 與欄位值，避免 iOS 因輸入節點被刪除或停用而關閉鍵盤。
11. 鍵盤 Enter／換行鍵呼叫表單 `requestSubmit()`；判定後等待 650ms 顯示回饋，再自動前往下一題，最後一題直接進入結果頁。
12. `Sound` 在建立及恢復 AudioContext 前設定 `navigator.audioSession.type = 'playback'`，並在五十音讀音的指標／鍵盤事件中重新解鎖音訊。
