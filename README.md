...
本版以 V1.3.1 為基底，針對已更新後仍出現的手機底部留白，再調整 App 的視窗配置。完整功能保留；沒有變更資料庫名稱、備份 Schema、Google OAuth、Drive 資料夾或通知後端設定。

## 這次更新

- 移除 V1.3.1 的 App fixed／inset 外框。主畫面 PWA 使用 100vh 作為根元素高度，App 及內容沿同一高度配置，避免底部列受固定定位的另一套視窗邊界影響。
- 一般瀏覽器維持動態視窗高度；iPad 分割視窗依視窗大小配置，不使用手機型號或 screen.height 硬算高度。
- 導覽按鈕保持至少 48px，縮短上下間距；底部只保留「4px 與系統安全區的較大值」，不再相加。Home indicator 的必要空間仍保留。
- 內容區保持獨立捲動；手寫操作列與寬螢幕側邊導覽保留。側邊導覽改定位於 App 內，使用同一高度基準。
...
7. 確認首頁右上角與設定頁目前版本均為 V1.3.2。若仍顯示舊版，回到首頁結束練習／備份後，在設定頁按「檢查更新」及「立即更新」。本版不必重新部署 Worker、初始化 D1，也不必重新產生 VAPID Keys。請勿清除網站資料或移除 PWA，以免影響尚未備份的本機紀錄。
...
系統可能沿用已安裝 PWA 的舊圖示；即使圖示尚未刷新，只要設定顯示 V1.3.2 就是新版。不要為更新圖示直接刪除 PWA 或網站資料。必要重裝前先備份並確認可以還原；重装後每台裝置須重新檢查通知訂閱。
...
部署後驗收：首頁資料正確、六種模式可切換、手寫可連續畫筆／評分／下一题、讀音 Enter 可送出與換題、設定資料保存仍在最下方、備份還原正常、各裝置測試通知成功、版本顯示 V1.3.2。
