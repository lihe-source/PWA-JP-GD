...
本版修正 Gemini 異常回覆被當成每日例句的問題。例句使用固定 JSON 結構，並在寫入前驗證日文、完整假名、繁體中文及推薦詞；不完整、截斷或不合格內容不會顯示、出題或同步。既有異常 AI 例句會被隔離，保留今天推薦的同一個單字重新建立例句。

從 V1.4.0 升級：先在舊版完成 Google Drive 備份，再將本包 46 個同層檔案覆寫至 GitHub main 根目錄。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.4.1。不可清除網站資料。本次是純前端更新，不需執行 `db:init`、部署 Worker、重建 D1 或更換 VAPID。

`npm run check` 檢查語法，`npm test` 執行日文版回歸與 10,001 筆歷史壓力測試。保留 V1.4.0 的手寫效能、貼底導覽、Google Drive、通知、OAuth、資料格式及設定。
