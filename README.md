...
本版讓整組五十音手寫完成後仍停留在同一頁。最後一題評分後會保留寫下的字，完整顯示該題總分與五項分數，並在同一成績區直接加入本次平均分數及達標題數，不再切換到另一個結果頁。

從 V1.4.3 升級：先在舊版完成 Google Drive 備份，再將本包 46 個同層檔案覆寫至 GitHub main 根目錄。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.4.4。不可清除網站資料。本次是純前端更新，不需執行 `db:init`、部署 Worker、重建 D1 或更換 VAPID。

`npm run check` 檢查語法，`npm test` 執行日文版回歸、最後一題同頁完成規則及 10,001 筆歷史壓力測試。保留 V1.4.3 的筆跡下方顯示分數、手寫效能、例句驗證、Google Drive、通知、OAuth、資料格式及設定。
