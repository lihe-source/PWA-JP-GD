...
本版以 V1.3.0 為基底，修正手機介面底部過多留白。完整功能保留；沒有變更資料庫名稱、備份 Schema、Google OAuth、Drive 資料夾或通知後端設定。

## 這次更新

- App 外框改以視窗四邊定位，取消額外的 100dvh 高度計算。
- 底部導覽列依按鈕內容自動決定高度，只在底部內距保留一次系統安全區，不再額外指定高度與 flex-basis。Home indicator 的必要空間仍保留。
- 內容區保持獨立捲動，不用浮動導覽覆蓋內容；手寫操作列及寬螢幕側邊導覽保留。
...
7. 確認目前版本為 V1.3.1。本版不必重新部署 Worker、初始化 D1，也不必重新產生 VAPID Keys。請勿清除網站資料或移除 PWA，以免影響尚未備份的本機紀錄。
...
系統可能沿用已安裝 PWA 的舊圖示；即使圖示尚未刷新，只要設定顯示 V1.3.1 就是新版。不要為更新圖示直接刪除 PWA 或網站資料。必要重裝前先備份並確認可以還原；重装後每台裝置須重新檢查通知訂閱。
...
部署後驗收：首頁資料正確、六種模式可切換、手寫可連續畫筆／評分／下一题、讀音 Enter 可送出與換題、設定資料保存仍在最下方、備份還原正常、各裝置測試通知成功、版本顯示 V1.3.1。
