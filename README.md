...
- 五十音讀音回饋：答對、答錯與練習結束使用和單字拼寫相同的音效；iOS 改用 `playback` 音訊工作階段，靜音模式下仍能輸出學習提示音。
...
- 低延遲手寫：Apple Pencil／觸控取樣以畫面更新頻率合併，只增量補畫新線段；限制過大的 Retina 畫布並快取畫布座標，降低 iPhone 與 iPad 長筆畫卡頓。
...
手機上傳的逐步畫面路徑請見 `MOBILE_UPLOAD_GUIDE.md`；系統架構與資料隔離請見 `ARCHITECTURE_V1_2_11.md`；本版變更請見 `CHANGELOG_V1_2_11.md`。
