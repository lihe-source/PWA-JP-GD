...
針對 iPad Air 與 iPhone 15 Pro Max 的長期使用與書寫停頓，本版將五十音手寫、讀音紀錄改成逐題增量寫入，不再每題重寫全部歷史；取消 2,500／5,000 筆截斷。手寫每批輸入只送出一次 Canvas 繪圖命令，診斷可分辨輸入延遲、事件處理、繪圖及主執行緒長工作。

從 V1.3.8 升級：將本包 46 個同層檔案覆寫至 GitHub main 根目錄，等待 Pages 發布。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.4.0。前端資料會自動升級到 IndexedDB Schema 2，不可清除網站資料。若要套用「每日最早完成時間」的 Worker 修正，再執行 `npm run worker:deploy`；不需重建 D1 或更換 VAPID。

`npm run check` 檢查語法，`npm test` 執行 98 項日文版回歸與 10,001 筆壓力測試。這不等於 iPad／iPhone 真機延遲測量。仍有問題時，可在手寫「更多設定」開啟書寫診斷，保留畫面上的輸入、事件、繪製及評分數據。
