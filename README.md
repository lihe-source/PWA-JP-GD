...
本版恢復五十音手寫「整組練習總結」。每一題按評分後仍在目前書寫頁顯示該字與六項分數；最後一題評分後改顯示「查看練習總結」，使用者確認後才進入總結頁，不會在評分當下突然換頁。

總結頁顯示平均分、達 80 分題數、最高分、需要加強題數及每題成績，並提供「再練一次」與「返回練習設定」。iPhone／矮螢幕只捲動成績清單，操作按鈕固定保留在清單下方；iPad 使用四格摘要。V1.4.8 的推薦詞完整五十音行驗證、舊快取攔截與 Gemini 自動重試完整保留。

從 V1.4.8 更新時，將本包 46 個同層檔案覆寫至 GitHub main 根目錄，等待 Pages 完成後到設定頁按「檢查更新」，確認版本為 V1.4.9。此修正不變更 Google Drive、OAuth、Cloudflare Worker、D1、通知訂閱、VAPID 或學習資料格式；不需執行 `db:init` 或重新部署 Worker。

回歸測試確認 50 題逐題評分不重建 Canvas，最後一題先保留分數，再由按鈕進入包含每題資料的總結；重練會清空本輪結果並建立新題序。完整語法、推薦詞行別與既有功能測試仍須全部通過。
