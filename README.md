...
針對 iPad Air 與 iPhone 15 Pro Max 書寫途中停頓：改為移動事件收到座標就直接增量繪圖；捕捉遺失後仍接續同一筆，真正取消或切到背景才結束。保留每題本機儲存、評分、自動發音、Pencil／手指選擇及已確認的貼底排版。

從 V1.3.7 升級：將本包 46 個同層檔案覆寫至 GitHub main 根目錄，等待 Pages 發布。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.3.8。不需重新部署 Cloudflare、重建 D1 或更換 VAPID。Worker 顯示 V1.3.7 正常。請勿為更新清除網站資料。

`npm run check` 檢查語法，`npm test` 執行日文版回歸測試（包含動畫影格完全不執行時筆跡仍更新）。這不等於 iPad／iPhone 真機延遲測量。仍有問題時，可在手寫「更多設定」開啟書寫診斷，保留畫面上的繪製與事件等待數據。
...
完成回報 API 與兩個 D1 輔助表屬既有功能。已可使用此功能者，本次 V1.3.8 更新不需執行下列步驟。首次建立此功能時，在專案根目錄依序執行：
