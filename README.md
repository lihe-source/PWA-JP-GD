...
本版以使用者已確認的 V1.3.3 為基底，移除五十音讀音答題畫面的假名類型與行別提示。保留貼底排版、資料、帳戶與通知設定。

## 這次更新

- 移除題目上方「平假名／片假名・行別」標籤，以及右上方的假名類型標籤。
- 進度、假名字形、拼音輸入與送出操作保留；標題列改為兩欄，不留下空的提示欄。
- 練習前仍可選平／片假名與行別；手寫練習的標籤、音效、讀音鍵盤流程、統計與备份不變。
- 上傳本包 46 個同層檔案後，確認版本為 V1.3.4。保留開啟時自動檢查更新及設定頁手動檢查功能；不必重設 Cloudflare。
...
7. 確認首頁右上角與設定頁目前版本均為 V1.3.4。若仍顯示舊版，回到首頁結束練習／備份後，在設定頁按「檢查更新」及「立即更新」。本版不必重新部署 Worker、初始化 D1，也不必重新產生 VAPID Keys。請勿清除網站資料或移除 PWA，以免影響尚未備份的本機紀錄。
...
系統可能沿用已安裝 PWA 的舊圖示；即使圖示尚未刷新，只要設定顯示 V1.3.4 就是新版。不要為更新圖示直接刪除 PWA 或網站資料。必要重裝前先備份並確認可以還原；重装後每台裝置須重新檢查通知訂閱。
...
部署後驗收：首頁資料正確、六種模式可切換、手寫可連續畫筆／評分／下一题、讀音 Enter 可送出與換題、設定資料保存仍在最下方、備份還原正常、各裝置測試通知成功、版本顯示 V1.3.4。
