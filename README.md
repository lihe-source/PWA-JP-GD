...
本版修正「Gemini 測試成功，但首頁重新產生先顯示錯誤；切到其他頁面再回首頁才出現單字／例句」的問題。從截圖可確認 API 已成功生成，推薦單字也已寫入本機；真正的錯誤發生在首頁接著啟動例句同步時，請求狀態放在錯誤的物件層級，造成已成功顯示的單字卡被後續 TypeError 改寫成失敗訊息。

推薦單字與例句的請求狀態已移入首頁 View。首頁載入、手動重試或切頁回來同時觸發時，相同設定只共用一個生成請求；舊請求不能覆蓋新畫面。推薦單字成功保存後會立即保留在當前頁面，後續例句生成即使失敗，也只更新單字卡內的例句狀態，不會再把整張推薦卡改成推薦失敗。

從 V1.4.6 更新時，將本包 46 個同層檔案覆寫至 GitHub main 根目錄，等待 Pages 完成後到設定頁按「檢查更新」，確認版本為 V1.4.7。此修正不變更 Google Drive、OAuth、Cloudflare Worker、D1、通知訂閱、VAPID 或學習資料格式；不需執行 `db:init` 或重新部署 Worker。

回歸測試新增：兩次同時重試只呼叫一次 Gemini、成功結果保留於當前首頁、首頁可直接建立並保存推薦例句、請求完成後狀態清空。完整語法與既有功能測試仍須全部通過。
