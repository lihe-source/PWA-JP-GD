...
本版改善 iPhone 15 Pro Max 的五十音手寫評分畫面。書寫時維持原本的大畫布；完成評分後切換為「成績優先」檢視，完整顯示總分、五項分數、建議與下一題按鈕。原筆跡預設收合，可按「查看完整筆跡」展開，且不會覆蓋底部導覽或操作按鈕。

從 V1.4.1 升級：先在舊版完成 Google Drive 備份，再將本包 46 個同層檔案覆寫至 GitHub main 根目錄。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.4.2。不可清除網站資料。本次是純前端更新，不需執行 `db:init`、部署 Worker、重建 D1 或更換 VAPID。

`npm run check` 檢查語法，`npm test` 執行日文版回歸、手機評分版面規則與 10,001 筆歷史壓力測試。保留 V1.4.1 的例句驗證、手寫效能、貼底導覽、Google Drive、通知、OAuth、資料格式及設定。
