...
本版改善 iPhone 15 Pro Max 的五十音手寫評分流程。書寫時維持原本的大畫布；完成評分後仍停留在同一題，剛寫完的筆跡會自動縮成清楚的唯讀預覽，完整總分、五項分數與建議直接顯示在筆跡下方，不必再按一次查看按鈕。

從 V1.4.2 升級：先在舊版完成 Google Drive 備份，再將本包 46 個同層檔案覆寫至 GitHub main 根目錄。開啟 App 會自動檢查更新；練習或儲存中會等安全時機才套用。也可在設定按「檢查更新／立即更新」，確認 V1.4.3。不可清除網站資料。本次是純前端更新，不需執行 `db:init`、部署 Worker、重建 D1 或更換 VAPID。

`npm run check` 檢查語法，`npm test` 執行日文版回歸、筆跡與分數相鄰規則及 10,001 筆歷史壓力測試。保留 V1.4.2 的例句驗證、手寫效能、貼底導覽、Google Drive、通知、OAuth、資料格式及設定。
