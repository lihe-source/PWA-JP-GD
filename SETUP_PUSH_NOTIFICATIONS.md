...
7. 若剛更新 VAPID Keys，重新按「儲存並啟用」；V1.2.6 會自動汰換舊的 Apple 訂閱。

若測試仍失敗，先確認 Worker 根網址顯示版本也是 `V1.2.6`，再執行 `npx wrangler tail japanese-daily-reminder --format pretty`。V1.2.6 會在紀錄中顯示 Apple 的 `providerReason`，並對 `BadDeviceToken` 自動刪除失效資料、由前端重建訂閱後重送一次。
