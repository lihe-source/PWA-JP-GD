...
test('English-style bottom navigation reserves its full height above practice content', async () => {
  ...
  const design = style.slice(style.indexOf('/* ===== V1.3.6 藍墨'));
...
test('all public app surfaces use Japanese V1.3.6', async () => {
  ...
  assert.match(app, /APP_VERSION = 'V1_3_6'/);
  assert.match(html, /app\.js\?v=V1_3_6/);
  assert.match(sw, /Japanese-PWA-V1_3_6/);
  ...
  assert.match(JSON.parse(manifest).name, /V1\.3\.6/);
  assert.equal(JSON.parse(pkg).version, '1.3.6');
});
...
test('V1.3.6 keeps Apple subscription repair and provider errors', async () => {
...
test('V1.3.6 adds kana-to-romaji practice under handwriting with statistics', async () => {
...
test('V1.3.6 recommends one daily word and stores its sentence practice', async () => {
  const [app, style, module, sw] = await Promise.all([
    text('app.js'), text('style.css'), text('daily-learning.js'), text('sw.js')
  ]);
  assert.match(app, /daily-learning\.js\?v=V1_3_6/);
  ...
  assert.match(sw, /daily-learning\.js\?v=V1_3_6/);
});
