...
test('English-style bottom navigation reserves its full height above practice content', async () => {
  ...
  const design = style.slice(style.indexOf('/* ===== V1.4.1 藍墨'));
...
test('all public app surfaces use Japanese V1.4.1', async () => {
  ...
  assert.match(app, /APP_VERSION = 'V1_4_1'/);
  assert.match(html, /app\.js\?v=V1_4_1/);
  assert.match(sw, /Japanese-PWA-V1_4_1/);
  ...
  assert.match(JSON.parse(manifest).name, /V1\.4\.1/);
...
test('V1.4.1 keeps Apple subscription repair and provider errors', async () => {
...
test('V1.4.1 adds kana-to-romaji practice under handwriting with statistics', async () => {
...
test('V1.4.1 recommends one daily word and stores its sentence practice', async () => {
  const [app, style, module, sw] = await Promise.all([
    text('app.js'), text('style.css'), text('daily-learning.js'), text('sw.js')
  ]);
  assert.match(app, /daily-learning\.js\?v=V1_4_1/);
  ...
  assert.match(sw, /daily-learning\.js\?v=V1_4_1/);
});

test('V1.4.1 rejects thought-only output and quarantines invalid AI sentence caches', async () => {
  const app = await text('app.js');
