...
test('the GitHub release directory is completely flat', async () => {
  ...
  else assert.equal(releaseEntries.length, 46, 'review deployment inventory before adding a file');
...
test('new install icons are opaque square PNGs and versioned in the manifest', async () => {
  ...
  for (const size of [192, 512]) {
    ...
    assert.ok(manifest.icons.some(icon => icon.src === `icon-${size}.png?v=V1_4_7`));
  }
