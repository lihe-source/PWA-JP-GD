...
test('failed flush prevents worker activation and reload; retry succeeds when safe', async () => {
  ...
  const updater = new VersionManager({ currentVersion: 'V1_3_4', storage: { async flush() { if (fail) throw new Error('quota'); } } });
...
test('starting a practice while flush is pending cancels activation', async () => {
  ...
  const updater = new VersionManager({ currentVersion: 'V1_3_4', canActivate: () => !active, storage: { async flush() { active = true; } } });
