...
test('failed flush prevents worker activation and reload; retry succeeds when safe', async () => {
  ...
  const updater = new VersionManager({ currentVersion: 'V1_5_0', storage: { async flush() { if (fail) throw new Error('quota'); } } });
...
test('starting a practice while flush is pending cancels activation', async () => {
  ...
  const updater = new VersionManager({ currentVersion: 'V1_5_0', canActivate: () => !active, storage: { async flush() { active = true; } } });
...
test('V1.5.0 retains more than ten thousand incremental practice records', () => {
